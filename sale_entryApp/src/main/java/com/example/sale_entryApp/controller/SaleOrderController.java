...
public class SaleOrderController {
    ...
    @PostMapping("/{salespersonId}/createCart/{customerId}")
    public ResponseEntity<?> createOrder(@PathVariable int salespersonId,@PathVariable int customerId){
        try {
    ...
    public ResponseEntity<?> addProductInSaleOrder(@PathVariable int productId,@PathVariable int orderId){
        try {
            OrderItem orderItem = orderItemService.addproduct(productId,orderId);
    ...
    //For Seeing all SalesOrders(For Now it's not Work like Found Sale Order For Particular Salesperson,it can be doable)
    @GetMapping("/viewOrders")
    public ResponseEntity<?> viewOrder(){
        try{
            List<SaleOrderDTO> orders=saleOrderService.viewSaleOrder();
    ...
    //For Finding All Orders Of Particular customer
    @GetMapping("/filter/customer/{customerId}")
    public ResponseEntity<?> filterByCustomer(@PathVariable int customerId){
        try{
            List<SaleOrderDTO> saleOrderDTOList =saleOrderService.findOrderByCustmerId(customerId);
            return ResponseEntity.status(HttpStatus.OK).body(saleOrderDTOList);
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
    }
    //For Finding All Orders By Particular Status
    @GetMapping("filter/status/{status}")
    public ResponseEntity<?> filterByStatus(@PathVariable String status){
        try{
         List<SaleOrderDTO> saleOrderDTOList=saleOrderService.findOrderByStatus(status);
            return ResponseEntity.status(HttpStatus.OK).body(saleOrderDTOList);
        }catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        }
    ...
    @GetMapping("/totalrev/{s_personId}")
    public Double calTotalSales(@PathVariable int s_personId){
        return saleOrderService.cal_revenue(s_personId);
    }
